...
nginx反向代理主要通过proxy_pass来配置，将你项目的开发机地址填写到proxy_pass后面，正常的格式为`proxy_pass URL`即可

```config
# http://127.0.0.1:80/api/test -> http://127.0.0.1:8082/api/test
location /api/ {
...
server {
    listen 80;
    location / {
        proxy_pass http://www.zhengqing520.com:81;  # 配置反向代理
    }
}
```

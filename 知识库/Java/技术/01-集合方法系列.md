...
### List按某个字段排序 - 支持：字符串（String）、日期（Date）、整型（Integer）、整形（Long）
...
Comparator.comparing 默认为升序排序
降序代码： Comparator.comparing(stu::getA,Comparator.reverseOrder())
...
// ex: 多个字段排序
 List<User> list = Lists.newArrayList(
        User.builder().id(2).sort(2).username("xx").build(),
        User.builder().id(5).sort(5).username("xxx").build()
);
System.out.println(list); // [User(id=2, sort=2, username=xx, nickname=null), User(id=5, sort=5, username=xxx, nickname=null)]
// 对商品sort降序,商品id降序
list = list.stream()
        .sorted(Comparator.comparing(User::getSort, Comparator.reverseOrder())
                .thenComparing(User::getId, Comparator.reverseOrder()))
        .collect(Collectors.toList());
System.out.println(list); // [User(id=5, sort=5, username=xxx, nickname=null), User(id=2, sort=2, username=xx, nickname=null)]
```

#### 6、list转map.

```
Map<Long, User> userMap = userList.stream().collect(Collectors.toMap(User::getUserId, t -> t,(k1,k2)->k1)); //（其中，若集合对象key有重，可根据(k1,k2)->k1设置<保留k1，舍弃k2>.）

ex: Map<Integer, Boolean> ifReadMap = docMsgList.stream().collect(Collectors.toMap(DocMsg::getDataId, DocMsg::getIfRead, (k1, k2) -> k1));
```
...
public class Java8SortTest {
    ...
    public void testSort() throws Exception {
        System.out.println("=================== ↓↓↓↓↓↓ 单字段排序 ↓↓↓↓↓↓ ===================");
        List<Integer> numList = Lists.newArrayList(1, 5, 6, 2);
        // 升序 -- 此方式需赋值
        numList = numList.stream().sorted().collect(Collectors.toList());
        System.out.println(numList); // [1, 2, 5, 6]
        // 升序
        numList.sort(Comparator.naturalOrder());
        System.out.println(numList); // [1, 2, 5, 6]
        // 降序
        numList.sort(Comparator.reverseOrder());
        System.out.println(numList); // [6, 5, 2, 1]


        System.out.println("\n=================== ↓↓↓↓↓↓ 单字段排序 -- 对象中的字段 ↓↓↓↓↓↓ ===================");
        List<User> list = Lists.newArrayList(
                User.builder().id(1).sort(2).name("张三").build(),
                User.builder().id(10).sort(2).name("张三2").build(),
                User.builder().id(2).sort(5).name("李四").build(),
                User.builder().id(3).sort(1).name("王五").build()
        );
        // 对sort升序
        list = list.stream().sorted(Comparator.comparing(User::getSort)).collect(Collectors.toList());
        System.out.println(JSON.toJSONString(list)); // [{"id":"3","name":"王五","sort":1},{"id":"1","name":"张三","sort":2},{"id":"10","name":"张三2","sort":2},{"id":"2","name":"李四","sort":5}]
        // 对sort降序
        list = list.stream().sorted(Comparator.comparing(User::getSort).reversed()).collect(Collectors.toList());
        System.out.println(JSON.toJSONString(list)); // [{"id":"2","name":"李四","sort":5},{"id":"1","name":"张三","sort":2},{"id":"10","name":"张三2","sort":2},{"id":"3","name":"王五","sort":1}]


        System.out.println("\n=================== ↓↓↓↓↓↓ 多字段排序 ↓↓↓↓↓↓ ===================");
        // 对sort升序 & sort相同时根据id降序
        list = list.stream()
                .sorted(Comparator.comparing(User::getSort)
                        .thenComparing(User::getId, Comparator.reverseOrder()))
                .collect(Collectors.toList());
        System.out.println(JSON.toJSONString(list));  // [{"id":"3","name":"王五","sort":1},{"id":"10","name":"张三2","sort":2},{"id":"1","name":"张三","sort":2},{"id":"2","name":"李四","sort":5}]

        // 对sort降序 & sort相同时根据id升序
        list = list.stream()
                .sorted(Comparator.comparing(User::getSort).reversed()
                        .thenComparing(User::getId))
                .collect(Collectors.toList());
        System.out.println(JSON.toJSONString(list)); // [{"id":2,"name":"李四","sort":5},{"id":1,"name":"张三","sort":2},{"id":10,"name":"张三2","sort":2},{"id":3,"name":"王五","sort":1}]

        System.out.println("\n=================== ↓↓↓↓↓↓ 空值处理 ↓↓↓↓↓↓ ===================");
        list.add(User.builder().id(100).sort(null).name("hi").build());
        // 升序时，属性值为空排前面
        list.sort(Comparator.comparing(User::getSort, Comparator.nullsFirst(Integer::compareTo)));
        System.out.println(JSON.toJSONString(list)); // [{"id":100,"name":"hi"},{"id":3,"name":"王五","sort":1},{"id":1,"name":"张三","sort":2},{"id":10,"name":"张三2","sort":2},{"id":2,"name":"李四","sort":5}]
        // 升序时，属性值为空排后面
        list.sort(Comparator.comparing(User::getSort, Comparator.nullsLast(Integer::compareTo)));
        System.out.println(JSON.toJSONString(list)); // [{"id":3,"name":"王五","sort":1},{"id":1,"name":"张三","sort":2},{"id":10,"name":"张三2","sort":2},{"id":2,"name":"李四","sort":5},{"id":100,"name":"hi"}]
        // 降序时，属性值为空排到后面
        list.sort(Comparator.comparing(User::getSort, Comparator.nullsFirst(Integer::compareTo)).reversed());
        System.out.println(JSON.toJSONString(list)); // [{"id":2,"name":"李四","sort":5},{"id":1,"name":"张三","sort":2},{"id":10,"name":"张三2","sort":2},{"id":3,"name":"王五","sort":1},{"id":100,"name":"hi"}]
        // 降序时，属性值为空排到前面
        list.sort(Comparator.comparing(User::getSort, Comparator.nullsLast(Integer::compareTo)).reversed());
        System.out.println(JSON.toJSONString(list)); // [{"id":100,"name":"hi"},{"id":2,"name":"李四","sort":5},{"id":1,"name":"张三","sort":2},{"id":10,"name":"张三2","sort":2},{"id":3,"name":"王五","sort":1}]
